...
def test_tdt_blank_duration_does_not_skip_next_frame_token() -> None:
    """Blank+duration advance must not trigger the RNN-T-style fallback time skip."""

    class BlankThenTokenJoint(_PrefixAwareTdtJoint):
        def forward(
            ...
            token_logits[..., 0] = 0.0
            duration_logits[..., 0] = 5.0

            time_value = int(enc[0, 0, 0].item())
            label_value = int(pred[0, 0, 0].item())
            if time_value == 0 and label_value == 0:
                token_logits[0, 0, 0, 0] = 5.0
                token_logits[0, 0, 0, 1] = -10.0
            elif time_value == 1 and label_value == 0:
                token_logits[0, 0, 0, 1] = 5.0
                token_logits[0, 0, 0, 0] = -10.0
            return token_logits, duration_logits

    decoder = _PrefixAwareDecoder()
    joint = BlankThenTokenJoint()
    enc = torch.zeros(1, 4, 4)
